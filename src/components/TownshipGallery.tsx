...
const TownshipGallery = () => {
  ...
  const townshipScenes = [
    { title: t("scene_shisa"), location: "Soweto", vibe: t("scene_shisa_vibe"), image: "https://images.unsplash.com/photo-1555939594-58d7cb561ad1?w=600&q=80", accent: "kasi-gold" },
    { title: t("scene_legends"), location: "Alexandra", vibe: t("scene_legends_vibe"), image: "https://images.unsplash.com/photo-1431324155629-1a6deb1dec8d?w=600&q=80", accent: "kasi-green" },
    { title: t("scene_tavern"), location: "Khayelitsha", vibe: t("scene_tavern_vibe"), image: "https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?w=600&q=80", accent: "kasi-orange" },
    { title: t("scene_street"), location: "Mamelodi", vibe: t("scene_street_vibe"), image: "https://images.unsplash.com/photo-1529156069898-49953e39b3ac?w=600&q=80", accent: "kasi-green" },
    { title: t("scene_matchday"), location: "Umlazi", vibe: t("scene_matchday_vibe"), image: "https://images.unsplash.com/photo-1574629810360-7efbbe195018?w=600&q=80", accent: "kasi-gold" },
    { title: t("scene_victory"), location: "Tembisa", vibe: t("scene_victory_vibe"), image: "https://images.unsplash.com/photo-1540747913346-19e32dc3e97e?w=600&q=80", accent: "kasi-orange" },
  ];

  const stats = [
    { icon: Users, value: "500K+", label: t("township_bettors") },
    { icon: Trophy, value: "R50M+", label: t("township_won") },
    { icon: Zap, value: "24/7", label: t("township_live") },
  ];
  ...
  return (
    <section className="section-padding bg-background relative overflow-hidden">
      ...
      <div className="container mx-auto relative z-10">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-kasi-gold/10 border-2 border-kasi-gold rounded-full mb-6">
            ...
            <span className="font-bold text-kasi-gold text-sm tracking-wider">{t("township_label")}</span>
          </div>
          <h2 className="font-display text-4xl md:text-6xl lg:text-7xl text-foreground mb-4">
            {t("township_title_1")} <span className="text-kasi-green">{t("township_title_2")}</span>
            <br />
            <span className="text-kasi-gold">{t("township_title_3")}</span>
          </h2>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">{t("township_subtitle")}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {townshipScenes.map((scene, index) => (
            <div key={index} onClick={() => setSelectedScene(scene)} className={`group relative cursor-pointer transform transition-all duration-500 hover:-translate-y-2 ${index === 0 ? 'md:col-span-2 md:row-span-2' : ''}`}>
              ...
              <div className={`relative bg-card border-4 border-border rounded-xl overflow-hidden ${index === 0 ? 'aspect-square md:aspect-[4/3]' : 'aspect-[4/3]'}`}>
                <img src={scene.image} alt={scene.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" loading="lazy" />
      ...
      {selectedScene && (
        <div className="fixed inset-0 z-50 bg-background/98 flex items-center justify-center p-4" onClick={() => setSelectedScene(null)}>
          <button onClick={() => setSelectedScene(null)} className="absolute top-6 right-6 p-3 rounded-xl bg-card border-2 border-border hover:border-kasi-green transition-colors z-10">
            <X size={24} className="text-foreground" />
          </button>
          <div className="max-w-4xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="relative rounded-2xl overflow-hidden border-4 border-kasi-green">
              <img src={selectedScene.image.replace("w=600", "w=1200")} alt={selectedScene.title} className="w-full max-h-[70vh] object-cover" />
