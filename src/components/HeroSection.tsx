  ...
  return (
    <section className="relative min-h-screen flex items-center pt-20 overflow-hidden pattern-overlay">
      ...
      <div className="absolute inset-0 z-0">
        <img src={heroBg} alt="Township football" className="w-full h-full object-cover opacity-20" />
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="animate-slide-up max-w-2xl">
            ...
            <h1 className="font-display text-5xl md:text-6xl lg:text-7xl leading-[0.95] mb-6">
              ...
              <br />
              <span className="gradient-text-gold">{t("hero_title_2")}</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {quickBets.map((bet, i) => (
                <Link
                  key={i}
                  to="/live"
                  className="group flex items-center gap-2 px-4 py-2 bg-muted/50 border-2 border-border rounded-xl hover:border-primary/50 hover:bg-primary/5 transition-all"
                >
            ...
            <div className="grid grid-cols-3 gap-4 pt-8 border-t-2 border-border">
              <div className="text-center sm:text-left">
                <p className="text-3xl md:text-4xl font-display text-foreground">{t("hero_stat_winners")}</p>
                <p className="text-sm text-muted-foreground uppercase tracking-wide">{t("hero_stat_winners_label")}</p>
              </div>
              <div className="text-center sm:text-left">
                <p className="text-3xl md:text-4xl font-display gradient-text-gold">{t("hero_stat_paid")}</p>
                <p className="text-sm text-muted-foreground uppercase tracking-wide">{t("hero_stat_paid_label")}</p>
              </div>
              <div className="text-center sm:text-left">
                <p className="text-3xl md:text-4xl font-display gradient-text">{t("hero_stat_payout")}</p>
            ...
            <div className="relative animate-float">
              ...
              <div className="absolute bottom-24 -left-8 glass px-4 py-2 border-2 border-accent/30">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-accent">{t("hero_instant")}</span>
                </div>
              </div>
            </div>

            {/* Live Community Feed */}
            <div className="w-full max-w-sm glass border-2 border-primary/20 p-4 rounded-xl">
                ...
                >
                  {liveFeed.map((item, i) => (
                    <div key={i} className="flex items-center justify-between h-10">
                      ...
                      <span className="text-sm font-bold gradient-text-gold">{item.amount}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="glass p-4 max-w-sm mx-auto lg:mx-0 border-2 border-primary/20">
            <div className="flex items-center justify-between">
              ...
              <div className="text-center flex-1">
                <p className="text-xs text-primary font-bold uppercase tracking-wider mb-1">🔴 {t("hero_live_from")}</p>
